...
using taints:
if you taint a resource you mark it to be vvrecreated in the next execution, in cli: terrafrom taint aws_instance.web_server to recreate an instance.
...
# terrafrom state
terrafrom.tfstate, in json(creates backups)
terraform show, for entire state
terraform state show aws_instance.web-server, that specific resource state
terraform state rm aws_instance.web-server, remove that state from the state file
